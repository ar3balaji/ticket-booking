...
<div class="left" align="right">
        <a href="addSchedule.php">Add a new schedule</a>
    </div>
...
if ($_SERVER['REQUEST_METHOD']== "POST") {
	...
	$cdquery=oci_parse($conn,"update workfor set theatreid='".$cd."',worktype='".$wtype."',starttime=TO_DATE('".$sdtm."','DD-Mon-YYYY HH24:MI:SS'),endtime=TO_DATE('".$edtm."','DD-Mon-YYYY HH24:MI:SS')where (EMPLOYEEID='".$eid."' or EMPLOYEEID=(SELECT EMPLOYEEID FROM MOVIEUSER M INNER JOIN EMPLOYEE E ON M.EMAILID=E.USERID WHERE LOWER(USERID)=LOWER('".$fname."')) AND SCHEDULEID='".$sch."' )")or die ('None');
	//$update_schedule = oci_execute($cdquery);
	...
	$cdquery2=oci_parse($conn,"select THEATREID  from workfor WHERE EMPLOYEEID='".$eid."' AND SCHEDULEID='".$sch."'");
	oci_execute($cdquery2);
	oci_fetch_all($cdquery2, $array2);
	$numberofrows2 = oci_num_rows($cdquery2);
	
	$cdquery3=oci_parse($conn,"SELECT EMPLOYEEID FROM workfor where theatreId='".$cd."' and worktype='".$wtype."' and starttime between TO_DATE('".$sdtm."','DD-Mon-YYYY HH24:MI:SS') and TO_DATE('".$edtm."','DD-Mon-YYYY HH24:MI:SS') and scheduleid!='".$sch."'");
	...
	$cdquery5=oci_parse($conn,"SELECT WORKTYPE FROM workfor where theatreId='".$cd."' and employeeid='".$eid."' and starttime between TO_DATE('".$sdtm."','DD-Mon-YYYY HH24:MI:SS') and TO_DATE('".$edtm."','DD-Mon-YYYY HH24:MI:SS') and scheduleid!='".$sch."'");
	...
	if($numberofrows2==0)
	{
		echo "<span style='color:red'>Update failed: Employee with the given schedule does not exist</span>";
	}
	else if($numberofrows>1)
	{
		//echo "here";
		if(isset($array["THEATREID"]))
		echo "<span style='color:red'>update failed as the employee is scheduled to work for another theatre with ID:</span>".$array["THEATREID"][0];
	}
	else if($numberofrows5>0)
	{
		
		if(isset($array5["WORKTYPE"]))
			echo "<span style='color:red'>update failed as the employee is assigned with the work -".$array5["WORKTYPE"][0]." during same time in same theatre.</span>";
	}
	else if($numberofrows3>0)
	{
		//echo "here";
		if(isset($array3["EMPLOYEEID"]))
			echo "<span style='color:red'>update failed as another employee with ID -".$array3["EMPLOYEEID"][0]." works during same time on same work type in same theatre.</span>";
	}
	else if($numberofrows4==0)
	{
		...
		{
		echo "<span style='color:red'>update failed as the workpreference for the employee don't permit to assign the task:</span>".$wtype;			
		}
		
	}
	else if (oci_execute($cdquery)) {
		oci_commit($conn);
		echo "<span style='color:green'>updated</span>";
	}
	else {
		echo "<span style='color:red'>Update failed as the user entered is not present</span>";
		//echo oci_error();
	}
...
?>
	...
	function validateForm() {
		...
		var sch = document.forms["register"]["sch"].value;
		...
		if(sch != null && sch != "")
		{
			if(sch.match(/^[0-9]+$/) === null)
			{
			$(register_sch).text("Enter Numbers only");			
			result = false;
			}
		}
		...
		if (sch == null || sch == "") {
			$(register_sch).text("Enter the schedule number");			
			result = false;				
		}
	...
	<form name="register" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" onsubmit="return validateForm()" method="POST">		
		<fieldset >
			...
			<legend align="top">Update Schedule</legend>
			...
			<div class='container'>
				<label for='sch' >Schedule ID*: </label><br>
				<input type='text' name='sch' id='sch' maxlength="50" />
				<span id='register_sch' class='error'></span>
			</div>			
