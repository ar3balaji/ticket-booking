...
if ($_SERVER['REQUEST_METHOD']== "POST") {
	...
	if($numberofrows2==0)
	{
		echo "Update failed: Employee with the given schedule does not exist";
	}
	else if($numberofrows>1)
	{
		//echo "here";
		if(isset($array["THEATREID"]))
		echo "update failed as the employee is scheduled to work for another theatre with ID:".$array["THEATREID"][0];
	}
	else if($numberofrows5>0)
	{
		
		if(isset($array5["WORKTYPE"]))
			echo "update failed as the employee is assigned with the work -".$array5["WORKTYPE"][0]." during same time in same theatre.";
	}
	else if($numberofrows3>0)
	{
		//echo "here";
		if(isset($array3["EMPLOYEEID"]))
			echo "update failed as another employee with ID -".$array3["EMPLOYEEID"][0]." works during same time on same work type in same theatre.";
	}
	else if($numberofrows4==0)
	{
		...
		{
		echo "update failed as the workpreference for the employee don't permit to assign the task:".$wtype;			
		}
		...
		echo "Update failed as the user entered is not present";
		//echo oci_error();
	}
